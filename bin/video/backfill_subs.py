...
def get_eng_subtitle_indices(subs_json: dict) -> dict:
    ...
    for i, stream in enumerate(streams):
        ...
        if lang.startswith('en') and codec in ('subrip', 'ass', 'ssa', 'text', 'webvtt'):
            eng_text_idx = i
    ...
    for i, stream in enumerate(streams):
        lang = stream.get('tags', {}).get('language', '').lower()
        codec = stream.get('codec_name', '')
        if lang.startswith('en') and codec not in ('subrip', 'ass', 'ssa', 'text', 'webvtt'):
            eng_image_idx = i
    ...
    try:
        ...
        for track in tracks:
            if (track.get('type') == 'subtitles' and
                track.get('properties', {}).get('language', '').lower().startswith('en') and
                track.get('properties', {}).get('codec_id', '') in ('S_VOBSUB', 'S_HDMV/PGS')):
...
def add_srt_to_mp4(target_mp4: Path, srt_file: Path, out_path: Path, 
                   default_subtitle: bool = False) -> None:
...
def handle_text_subs(target_mp4: Path, mkv_file: Path, eng_text_idx: int, 
                     out_path: Path, default_subtitle: bool = False) -> None:
    """Handle English text subtitles by muxing them directly."""
    cmd = [
        'ffmpeg', '-y',
        '-i', str(target_mp4),
        '-i', str(mkv_file),
        '-map', '0', '-map', f'1:{eng_text_idx}',
        '-c', 'copy', '-c:s', 'mov_text',
...
def handle_image_subs_ocr(target_mp4: Path, mkv_file: Path, eng_img_idx: int, 
                        eng_img_codec: str, eng_img_tid: int, dst_dir: Path) -> None:
    ...
    if eng_img_codec == 'dvd_subtitle':
        ...
        if mkvextract_check.returncode == 0:
            try:
                subprocess.run(
                    ['mkvextract', 'tracks', str(mkv_file), f'{eng_img_tid}:"{tmp_base}.{sub_ext}'],
                ...
                if sub_path.exists() and idx_path.exists():
                    ...
                    print("Use these files with an OCR tool to create an SRT, then re-run this command.")
                else:
        ...
        if mkvextract_check.returncode == 0:
            try:
                subprocess.run(
                    ['mkvextract', 'tracks', str(mkv_file), f'{eng_img_tid}:"{tmp_base}.sup'],
                    capture_output=True, check=True
                )
                sup_path = Path(str(tmp_base) + '.sup')
                if sup_path.exists():
                    print(f"Extracted PGS file: {sup_path}")
                    print("Use this file with an OCR tool to create an SRT, then re-run this command.")
                else:
    ...
    # Clean up temp files
    for pattern in [str(tmp_base) + ".idx", str(tmp_base) + ".sub", str(tmp_base) + ".srt", str(tmp_base) + ".sup"]:
        try:
            Path(pattern).unlink()
...
def main():
    ...
    if not target_mp4:
        print(f"Could not uniquely determine target MP4. Expected: {dst_dir.name}.mp4 or exactly one MP4 in {dst_dir}")
        return 1
    ...
    if subtitle_info['eng_text_idx'] != -1:
        ...
        handle_text_subs(
            target_mp4, largest_mkv, subtitle_info['eng_text_idx'], 
            out_path, default_subtitle=args.inplace
        )
        if args.inplace:
            ...
            print(f"Replaced original MP4 with new file containing English subs. Backup at: {backup_path}")
        return 0
    ...
    handle_image_subs_ocr(
        target_mp4, largest_mkv, subtitle_info['eng_image_idx'], 
        subtitle_info['eng_image_codec'], subtitle_info['eng_img_tid'], dst_dir
    )
