...
def log_skipped_file(artist: str, file_path: Path, extracted_title: str, reason: str) -> None:
    ...
    with open(SKIPPED_LOG, 'a', newline='', encoding='utf-8') as f:
        ...
        if not file_exists:
            writer.writerow(['timestamp', 'artist', 'file_path', 'extracted_title', 'reason'])
...
def extract_title_from_filename(file_path: Path, artist_name: str) -> Optional[str]:
    ...
    title = re.sub(rf'^{re.escape(artist_name)}\s*[-–]\s*', '', filename, flags=re.IGNORECASE)
    
    # Remove common suffixes
    title = re.sub(r'\s*(?:\(.*?\)|\[.*?\])$', '', title)  # Remove brackets
    title = re.sub(r'\s*(?:official\s+video|music\s+video|video|hd|4k|1080p|720p)\s*$', '', title, flags=re.IGNORECASE)
    ...
    try:
        ...
        for rec in result.get('recording-list', []):
            if rec.get('artist-credit'):
                ...
                if (rec_artist.lower() == artist.lower() or 
                    artist.lower() in rec_artist.lower() or 
                    rec_artist.lower() in artist.lower()):
        ...
        try:
            ...
            for score, recording_id, title_guess, artist_guess in results:
                if score > 0.5 and recording_id:  # Lower threshold for video
                    # More permissive artist matching
                    if (artist_guess.lower() == artist.lower() or 
                        artist.lower() in artist_guess.lower() or 
                        artist_guess.lower() in artist.lower()):
                        try:
                            rec = musicbrainzngs.get_recording_by_id(recording_id, includes=["artists"])
...
def process_video_file(file_path: Path, artist_name: str, dry_run: bool = False, force: bool = False, use_extracted: bool = False) -> str:
...
def scan_music_videos(root_path: Path, dry_run: bool = False, force: bool = False, use_extracted: bool = False) -> Tuple[int, int, int]:
...
def main():
    ...
    changed, unchanged, skipped = scan_music_videos(root_path, args.dry_run, args.force, args.use_extracted)
