...
def fix_unknown_album(album_path: Path, correct_artist: str, correct_album: str, 
    ...
    if len(track_titles) != len(flac_files):
        print(f"Warning: MusicBrainz has {len(track_titles)} tracks but found {len(flac_files)} files")
        # Adjust to match file count
        if len(track_titles) < len(flac_files):
            track_titles.extend([f"Track {i+1:02d}" for i in range(len(track_titles), len(flac_files))])
        else:
...
def main():
    ...
    parser.add_argument(
        "album_path",
        type=Path,
        help="Path to unknown album folder"
    )
    parser.add_argument(
        "artist",
        help="Correct artist name"
    )
    parser.add_argument(
        "album",
        help="Correct album name"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
