...
def scan_directory(root_dir, dry_run=False):
    ...
    for root, dirs, files in os.walk(root_dir):
        ...
        if not dirs and not any(Path(f).suffix.lower() in {'.flac', '.mp3', '.m4a', '.wav', '.aac', '.ogg'} for f in files):
            continue
        
        # Check if this directory has audio files
        audio_files = [f for f in files if Path(f).suffix.lower() in {'.flac', '.mp3', '.m4a', '.wav', '.aac', '.ogg'}]
