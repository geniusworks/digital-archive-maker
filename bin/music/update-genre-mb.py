...
UNRESOLVED_FILE = Path(__file__).parent.parent.parent / "log" / "unresolved_genres.txt"
UNRESOLVED_FILES: set = set()
...
def add_unresolved_file(flac_path: Path, artist: str, album: str, title: str):
    ...
    if flac_path and artist and album:
        # Store as: file_path | artist - album - title
        entry = f"{flac_path} | {artist} - {album} - {title}"
        UNRESOLVED_FILES.add(entry)
...
def add_rejected_genre(genre: str, artist: str, album: str):
    ...
    if genre and genre.strip():
        # Store only the genre name, not the artist/album context
        # This prevents duplicates like "compilation (artist1)" and "compilation (artist2)"
...
def get_genre_from_musicbrainz(
    ...
    *,
    ...
    try:
        ...
        if not bypass_cache and cache_key_album in GENRE_CACHE:
            if return_source:
                return (cached, "cache")
            return cached
        ...
        if not normalize_tag_value(album):
            if not bypass_cache and cache_key_artist in GENRE_CACHE:
        ...
        if result.get("release-group-list"):
            ...
            try:
                ...
                if genre_tags:
                    ...
                    # Apply genre transformations before validation
                    genre = _transform_genre(genre)
                    ...
                    if christmas_genre and genre:
                        ...
                        if christmas_genre != genre:
                            genre = christmas_genre
                        # If they're the same, keep as is
        ...
        final_genre = christmas_genre if christmas_genre else ""
        GENRE_CACHE[cache_key_album] = final_genre
        if not normalize_tag_value(album):
