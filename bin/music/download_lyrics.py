...
class LyricsDownloader:
    def __init__(self, genius_token: Optional[str] = None, retry_failed: bool = False):
        ...
        self.fallback_access_failures = (
            0  # Consecutive lyrics.ovh access failures (timeouts/errors, not 404s)
        )
    ...
    def _search_lyrics_ovh(
        self, artist: str, title: str, indent: bool = False
    ...
    def _get_artist_variations(self, artist: str, title: str) -> List[Tuple[str, str]]:
        ...
        for var_artist, var_title in variations:
            if (
                var_artist
                and (var_artist, var_title) not in seen
                and (var_artist, var_title) != (artist, title)
            ):
    ...
    def download_lyrics_for_file(
        self, file_path: Path, force: bool = False, indent: bool = False
    ) -> Optional[bool]:
        ...
        if self.genius and not genius_on_cooldown:
            genius_result, genius_api_unavailable = self._search_genius(
                search_artist, title, indent
            )
            if genius_result:
        ...
        if not lyrics:
            ...
            (
                ...
                successful_artist,
                variation_success,
            ) = self._try_artist_variations(artist, title, indent)
        ...
        if not lyrics:
            ...
            genius_actually_searched = (
                self.genius and not genius_api_unavailable and not genius_on_cooldown
            )
    ...
    def _process_single_directory(self, directory: Path, audio_extensions: set, force: bool):
        ...
        print(
            f"  Success rate: {files_successful/files_processed*100:.1f}%"
    ...
    def _process_albums_recursively(self, directory: Path, force: bool):
        ...
        for album_dir in album_dirs:
            ...
            (
                ...
                lyrics_downloaded,
                quota_failures,
            ) = self._process_album(album_dir, audio_extensions, force)
    ...
    def _find_album_directories(self, directory: Path, audio_extensions: set) -> List[Path]:
        ...
        for item in directory.rglob("*"):
            if item.is_dir():
                ...
                has_audio = any(
                    f.is_file() and f.suffix.lower() in audio_extensions for f in item.glob("*")
                )
                if has_audio:
    ...
    def _process_album(
        ...
        if self.retry_failed:
            ...
            if audio_files and any(get_failure_count(f) > 0 for f in audio_files):
                print(
                    f"    📊 Prioritizing {len(audio_files)} tracks by failure count (least first)"
                )
    ...
    def _process_failed_tracks_globally(self, directory: Path, force: bool):
        ...
        for failure_count, file_path, artist, title in failed_tracks:
            ...
            print(
                f"    📈 Failed {failure_count} time{'s' if failure_count != 1 else ''} previously"
            )
