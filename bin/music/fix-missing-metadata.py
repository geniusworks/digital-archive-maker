...
def fetch_musicbrainz_metadata(artist: str, album: str, title: str) -> Dict:
    ...
    result = mb_call(musicbrainzngs.search_releases, 
                    artist=artist, release=album, limit=1)
    
    if not result:
        # Fallback: search by artist and title
        result = mb_call(musicbrainzngs.search_recordings, 
                        artist=artist, query=title, limit=1)
    ...
    release_result = mb_call(musicbrainzngs.get_release_by_id,
...
def update_audio_metadata(file_path: Path, new_title: str, artist: str = None, album: str = None, track_number: str = None) -> bool:
...
def scan_directory(directory: Path, recursive: bool = True) -> List[Tuple[Path, Dict[str, str], str]]:
...
def main():
    ...
    parser.add_argument("--recursive", action="store_true", default=True, help="Scan recursively (default)")
    parser.add_argument("--no-recursive", dest="recursive", action="store_false", help="Don't scan recursively")
    parser.add_argument("--verbose", action="store_true", help="Show detailed output")
    parser.add_argument("--no-musicbrainz", action="store_true", help="Disable MusicBrainz lookup (use directory structure only)")
    parser.add_argument("--cache-dir", help="Cache directory for MusicBrainz data", default="log")
    ...
    for file_path, metadata, issue_type in problematic_files:
        ...
        needs_update = (
            ...
            final_artist != current_artist or
            final_album != current_album or
            (final_track_number and final_track_number != metadata.get('tracknumber', ''))
        )
        ...
        success = update_audio_metadata(file_path, final_title, final_artist, final_album, final_track_number)
        if success:
            fixed_count += 1
            status = "✅"
        else:
        ...
        if args.verbose or not success:
            ...
            if final_track_number and final_track_number != metadata.get('tracknumber', ''):
                changes.append(f"Track: {repr(metadata.get('tracknumber', ''))} → {repr(final_track_number)}")
