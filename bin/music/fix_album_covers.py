...
def require_command(cmd: str) -> None:
    ...
    if result.returncode != 0:
        print(
            f"Error: Required command '{cmd}' not installed. Please install it.",
            file=sys.stderr,
        )
        sys.exit(1)
...
def process_album_dir(album_dir: Path) -> None:
    ...
    if artist_name in ("Unknown", "Untitled") or album_name in (
        "Unknown",
        "Untitled",
    ):
...
def main():
    ...
    parser.add_argument(
        "search_path",
        nargs="?",
        help="Path to album or library root directory",
    )
