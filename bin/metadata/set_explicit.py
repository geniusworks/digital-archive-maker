...
def process_single_file(flac_file: Path, value: str) -> None:
    ...
    if not flac_file.is_file() or flac_file.suffix.lower() != '.flac':
        print(f"Error: {flac_file} is not a FLAC file (use --album for directories)", file=sys.stderr)
        sys.exit(1)
...
Examples:
  ...
  %(prog)s /path/to/album Unknown --album
    ...
    parser.add_argument("value", choices=["Yes", "No", "Unknown"], 
                       help="EXPLICIT tag value")
