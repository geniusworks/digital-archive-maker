...
def match_flac_to_track(flac_path: Path, tracks: List[Tuple[str, str]]) -> Optional[Tuple[str, str]]:
...
def repair_album_tags(album_path: Path, dry_run: bool = False) -> int:
    ...
    for flac_path in flac_files:
        ...
        print(f"    Current: ARTIST='{current_tags.get('artist', '')}' ALBUM='{current_tags.get('album', '')}' TITLE='{current_tags.get('title', '')}'")
        ...
        if expected_title and current_tags.get('title', '') != expected_title:
            needs_repair = True
            differences['title'] = (current_tags.get('title', ''), expected_title)
        ...
        if expected_track and current_tags.get('tracknumber', '') != expected_track:
            needs_repair = True
            differences['tracknumber'] = (current_tags.get('tracknumber', ''), expected_track)
