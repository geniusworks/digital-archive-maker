...
## Overview
- Primary audio workflow uses `abcde` to rip CDs to `.flac`, then normalizes filenames, playlists, and cover art.
...
## Directory layout
- `_scripts/` (this directory)
  - `.abcde.conf` — abcde configuration for ripping CDs to `FLAC` at `OUTPUTDIR=/Volumes/Data/Media/Rips/CDs`
  - `_install/`
  ...
  - `albums.txt`, `artists.txt` — sample reports/outputs from other utilities.
  - `only_in_new.txt`, `only_in_old.txt` — difference lists written by compare scripts when not grouping.
