...
class TestBackfillSubs:
    ...
    def test_get_eng_subtitle_indices_no_english(self):
        ...
        subs_json = {"streams": [{"index": 1, "codec_name": "subrip", "tags": {"language": "spa"}}]}
    ...
    def test_probe_subtitle_streams_success(self, mock_subprocess, sample_mkv_dir):
        ...
        mock_subprocess.return_value.stdout = json.dumps(
            {"streams": [{"index": 1, "codec_name": "subrip", "tags": {"language": "eng"}}]}
        )
                ...
                "tracks": [
                    {
                        "id": 2,
                        "type": "subtitles",
                        "properties": {"language": "eng", "codec_id": "S_VOBSUB"},
                    }
                ]
            }
        )
    ...
    @patch("subprocess.run")
    def test_handle_image_subs_ocr_dvd(
        self, mock_subprocess, temp_dir, sample_mkv_dir, sample_mp4_dir
    ):
        """Test OCR handling for DVD subtitles."""
        dst_dir = sample_mp4_dir.parent

        with (
            patch("backfill_subs.os.getpid", return_value=12345),
    ...
    @patch("subprocess.run")
    def test_handle_image_subs_ocr_bluray(
        self, mock_subprocess, temp_dir, sample_mkv_dir, sample_mp4_dir
    ):
    ...
    def test_main_text_subs_success(
        self,
        mock_handle_text,
        mock_probe,
        mock_determine,
